...
void warpTriangle(Mat &img1, Mat &img2, vector<Point2f> &t1, vector<Point2f> &t2)
{
    ...
    {
        t1Rect.push_back({t1[i].x - static_cast<float>(r1.x), t1[i].y - static_cast<float>(r1.y)});
        t2Rect.push_back({t2[i].x - static_cast<float>(r2.x), t2[i].y - static_cast<float>(r2.y)});
